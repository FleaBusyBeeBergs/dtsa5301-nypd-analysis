...
loc_plot <- ggplot(nypd_df, aes(BORO, fill = VIC_AGE_GROUP)) +
  ...
  ylab('Count') +
  labs(fill = 'Age Group') +
  ggtitle('Locations of New York City shooting incidents, 2006 - 2022')
```
...
* 2011 to 2019 - decreasing
  ...
  * Was there a programme implemented to reduce gun violence?

* 2020 to 2021 - large increase

  * Were there prevention programmes that stopped during this time?
  * Around the same time as Covid-19 lockdowns - coincidence?
...
year_month_plot <- ggplot(nypd_df, aes(x = Month, colour = factor(Year), group = factor(Year))) +
  ...
  labs(title = 'Interactive plot: Monthly NYC shooting incidents by year',
...
* Weekly and daily counts vs. temperature data?
...
* shootings = -30.2 * (2 * pi * month)  -  33.4 * (2 * pi * month) + 129.5
